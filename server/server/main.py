...
async def start_bot_process(room_url: str, token: str, interview_meta: Dict[str, Any] | None = None) -> int:
    ...
    try:
        ...
        # Pass interview metadata to the bot via environment variables
        meta = interview_meta or {}
        if meta.get("template"):
            env["INTERVIEW_TEMPLATE"] = str(meta["template"])[:512]
        if meta.get("language"):
            env["INTERVIEW_LANGUAGE"] = str(meta["language"])[:32]
        if meta.get("questions"):
            # Accept list or newline‑separated string; keep size reasonable
            if isinstance(meta["questions"], list):
                q_str = "\n".join([str(q) for q in meta["questions"] if q])
            else:
                q_str = str(meta["questions"])
            env["INTERVIEW_QUESTIONS"] = q_str[:8000]
...
async def start_agent(request: Request):
    ...
    # Parse optional interview metadata from query
    qp = request.query_params
    meta = {
        "template": qp.get("template"),
        "language": qp.get("language"),
        "questions": qp.get("questions"),
    }
    # Start bot and redirect to room
    await start_bot_process(room_url, token, interview_meta=meta)
...
async def rtvi_connect(request: Request) -> Dict[str, Any]:
    ...
    # Parse optional interview metadata from body
    body = {}
    try:
    ...
    pid = await start_bot_process(room_url, token, interview_meta=meta)
    return {
        "room_url": room_url,
        "token": token,
        "bot_pid": pid,
