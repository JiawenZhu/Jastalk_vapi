...
class BotConfig:
    def __init__(self):
        ...
        # Validate required vars (only those always required for server startup)
        daily_api_key = os.getenv("DAILY_API_KEY")
        if not daily_api_key:
            raise ValueError("Missing required env var: DAILY_API_KEY")
    ...
    @property
    def google_credentials(self) -> str | None:
        """Raw JSON credentials for Google Cloud services (optional)."""
        return os.getenv("GOOGLE_CREDENTIALS_JSON")

    @property
    def google_credentials_path(self) -> str | None:
        """Path to service account JSON for Google Cloud STT/TTS."""
    ...
    def llm_provider(self, value: str):
        value = value.lower()
        if value not in ("google", "openai", "gemini-live"):
    ...
    def google_model(self) -> str:
        ...
        return os.getenv("GOOGLE_MODEL", "models/gemini-2.0-flash-live-001")
    ...
    def tts_provider(self) -> str:
        return os.getenv("TTS_PROVIDER", "google").lower()

    @tts_provider.setter
    def tts_provider(self, value: str):
        value = value.lower()
        if value not in ("google", "deepgram", "cartesia", "elevenlabs", "rime"):
            raise ValueError(f"Invalid TTS provider: {value}")

        os.environ["TTS_PROVIDER"] = value

    @property
    def google_tts_voice_id(self) -> str:
        return os.getenv("GOOGLE_TTS_VOICE_ID", "en-US-Chirp3-HD-Charon")

    @google_tts_voice_id.setter
    def google_tts_voice_id(self, value: str):
        os.environ["GOOGLE_TTS_VOICE_ID"] = value
